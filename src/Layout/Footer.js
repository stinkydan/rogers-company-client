import React, { Component } from 'react'

class Footer extends Component {
  render() {
    return(
      <div className="footer-container">
        <div className="footer-icon-container">
          <div className="footer-1">
            <svg preserveAspectRatio="xMidYMid meet" data-bbox="31 20.6 135.683 158.676" viewBox="31 20.600000381469727 135.68299865722656 158.67599487304688" xmlns="http://www.w3.org/2000/svg">
            <g><path fill="#FCFCFC" d="M102 20.685l-.5-.085-65.186 11.141v45.237H31v58.084h6.99l-.369.539 63.879 43.675 65.183-44.568V31.741zM37 82.978h28.014v46.084H37zm123.679 48.564L101.5 172.008l-54.037-36.946h23.551v-8.2a46.476 46.476 0 0 0 6.766 5.306c3.67 2.343 17.759 2.983 28.551 2.983 4.784 0 8.921-.126 11.215-.284 4.626-.32 12.368-.854 14.293-8q1.272-4.725 2.536-9.449 2.049-7.651 4.117-15.3a29.54 29.54 0 0 0 1.46-9.969c-.34-4.724-2.034-8.294-5.18-10.916a14.581 14.581 0 0 0-9.749-3.226h-15.176a37.173 37.173 0 0 0 4.725-20.571c-.059-1.143-.16-3.057-10.8-11.984l-.229-.174a7.53 7.53 0 0 0-11.763 5.445c-.687 6.872-2.731 16.656-8.566 24.241C79.647 79.6 76.3 81.8 71.668 84.835l-.654.432v-8.289h-28.7V36.8L101.5 26.686 160.679 36.8zm-35.667-47.537h.011a8.741 8.741 0 0 1 5.908 1.835c1.852 1.544 2.817 3.684 3.037 6.737a24.208 24.208 0 0 1-1.267 7.966q-2.079 7.654-4.122 15.318-1.264 4.72-2.533 9.441c-.615 2.284-2.781 3.153-8.913 3.575-12.4.855-33.337-.172-36.123-1.769a45.237 45.237 0 0 1-9.312-8.136q.02-13.474-.006-26.949c1.143-.778 2.228-1.489 3.267-2.171 4.736-3.107 8.827-5.791 13.011-11.229 6.7-8.706 9.015-19.653 9.781-27.3a1.5 1.5 0 0 1 .873-1.229 1.516 1.516 0 0 1 1.426.07c3.5 2.944 7.548 6.712 8.554 8.115.218 5.323-.5 11.979-6.552 21.064l-3.11 4.664z"></path></g>
            </svg>
            <span>Great<br/>Reviews</span>
          </div>
          <div className="footer-2">
            <svg preserveAspectRatio="xMidYMid meet" data-bbox="18.85 16.862 162.15 162.15" viewBox="18.850324630737305 16.861902236938477 162.14999389648438 162.14976501464844" xmlns="http://www.w3.org/2000/svg">
              <g><path fill="#ffffff" d="M99.7 16.862A81.075 81.075 0 1 0 181 97.716a81.168 81.168 0 0 0-81.3-80.854zm.426 156.15A75.075 75.075 0 1 1 175 97.732a75.159 75.159 0 0 1-74.871 75.28z"></path>
              <path fill="#ffffff" d="M114.553 80.153L99.782 45.715 85.2 80.234l-33.766.123 24.746 23.992-5.631 33.383 29.433-18.755 29.535 18.594-5.817-33.352 24.62-24.127zm6.734 45.147l-21.325-13.424-21.251 13.54 3.911-23.178L66.185 86.3l23-.084 10.64-25.186L110.6 86.16l23-.041-16.35 16.024z">
              </path>
              </g>
            </svg>
            <span>Five Star<br/>Service</span>
          </div>
          <div className="footer-3">
            <svg preserveAspectRatio="xMidYMid meet" data-bbox="25.25 19.497 149.631 160.88" viewBox="25.25 19.49700164794922 149.63099670410156 160.8800048828125" xmlns="http://www.w3.org/2000/svg">
              <g>
                <path fill="#ffffff" d="M174.881 118.377a32.924 32.924 0 0 0-29.631-32.718V28.377h-36.593l-3.775-8.88H65.619l-3.776 8.88H25.25v152h120V151.1a32.926 32.926 0 0 0 29.631-32.723zM69.587 25.5h31.326l4.8 11.294H64.786zM31.25 174.377v-140h28.042l-3.578 8.414h59.071l-3.577-8.414h28.042V85.62a32.872 32.872 0 0 0 0 65.515v23.242zM142 145.259a26.881 26.881 0 1 1 26.881-26.882A26.912 26.912 0 0 1 142 145.259z"></path>
                <path fill="#ffffff" d="M138.036 122.6l-7.02-5.7-3.784 4.656 8.871 7.207a3.385 3.385 0 0 0 4.594-.28l16.336-16.9-4.314-4.17z">
                </path>
                <path fill="#ffffff" d="M112 72.503v5.994H44v-5.994h68z"></path>
                <path fill="#ffffff" d="M88.909 102.253v6H44v-6h44.909z"></path>
                <path fill="#ffffff" d="M75.091 130.503v7H44v-7h31.091z"></path>
              </g>
            </svg>
            <span>Excellent<br/>Communication</span>
          </div>
        </div>
        <div className="footer-contact">
          <div>
            links
          </div>
          <div>
            <span>20 Anderson St Boston, MA 02114</span>
          </div>
          <div>
            <span><strong>Email:</strong> rogerdunn@gmail.com</span>
            <span><strong>Phone:</strong> 617-910-8271</span>
          </div>
          <div>
            <span>© 2018  Roger's Landscaping Service</span>
          </div>
        </div>
      </div>
    )
  }
}

export default Footer
